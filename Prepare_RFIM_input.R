...
phenotype = c('Asthma','Breast_cancer','Lung_cancer')
gwas_size = c('old','new')
...
for (thre in 1:6){
  for (k in 1:3){
    for (j in 2:2){
      p_old = read.csv(paste("../data/7_Dandi/",phenotype[k],"/",phenotype[k],"_combined.csv",sep = ""),header = FALSE,sep = ',')
      # remove na p-values
      ...
      write.table(stats_expression, paste("../data/RFIM_Dandi/",phenotype[k],"_",gwas_size[j],"_ppirevise_string_v3_f_",ff[thre],".txt",sep = ""), col.names=NAMES, sep=" ",quote = FALSE, row.names = FALSE)
      NAMES = colnames(node_info)
      NAMES[1] <- paste0("#", NAMES[1])
      write.table(node_info, paste("../data/RFIM_Dandi/",phenotype[k],"_",gwas_size[j],"_ppirevise_string_v3_f_",ff[thre],".txt",sep = ""), col.names=NAMES, sep=" ",quote = FALSE, row.names = FALSE,append = TRUE)
      NAMES = colnames(ppi)
      NAMES[1] <- paste0("#", NAMES[1])
      write.table(ppi, paste("../data/RFIM_Dandi/",phenotype[k],"_",gwas_size[j],"_ppirevise_string_v3_f_",ff[thre],".txt",sep = ""), col.names=NAMES, sep=" ",quote = FALSE, row.names = FALSE,append = TRUE)
    }
  }
}
