...
//////////////////////////////////////////////////////////////////////////////////////////////////////
/*int* Window::slicecount(int dir)
{
    // create a vector containing a count of +1's in slice given
    // by dir = 0, 1, 2
    int *vec;
    int spincount;
	...
	case 0:
		...
		for (int k = 0; k < H; ++k)
		    for (int j = 0; j < L; ++j) {
			spincount = i + W * (j + L * k);
			if (spin[spincount] == SPINUP) vec[i]++;
		    }
	    }
	    break;
	case 1:
		...
		for (int k = 0; k < H; ++k)
		    for (int i = 0; i < W; ++i) {
			spincount = i + W * (j + L * k);
			if (spin[spincount] == SPINUP) vec[j]++;
		    }
	    }
	    break;
	case 2:
		...
		for (int j = 0; j < L; ++j) 
		    for (int i = 0; i < W; ++i) {
			spincount = i + W * (j + L * k);
			if (spin[spincount] == SPINUP) vec[k]++;
		    }
	    }
	    break;
	default:
	    fprintf(stderr, "Bad direction for slicecount (%d)\n", dir);
	    exit(1);
	    break;
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
int* Window::slicecountcenter(int dir)
{
    // create a vector containing a count of +1's in slice given
    // by dir = 0, 1, 2
    // count in the middle 1/4 of the sample area
    int *vec;
    int spincount;
	...
	case 0:
	    ...
	    for (int i = 0; i < W; ++i) {
		vec[i] = 0;
		for (int k = H/4; k < 3*H/4; ++k)
		    for (int j = L/4; j < 3*L/4; ++j) {
			spincount = i + W * (j + L * k);
			if (spin[spincount] == SPINUP) vec[i]++;
		    }
	    }
...
/*/////////////////////////////////////////////////////////////////////////////////////////////////////
// NOTE: depthtrace assumes periodic in y, z directions
int Window::wallarea(int startboundary) // count up how many bonds on wall, moving
					// through regions of zeros starting from the indexed
					// boundary wall 
{
    //   temporarily mark up the window spin set for tracking the connected piece
    //   from that wall. 
    //   depth first search:
    //   mark visited sites with 2's
    //   move onto neighbor sites with value 0
    //   count up exposed faces when done.
    
    int surfarea;

    switch (startboundary) {
	case 0:			// LEFT
	    surfarea = depthtrace(W - 1, L - 1, H - 1);
	    break;
	case 1:			// RIGHT
	    surfarea = depthtrace(0, 0, 0);
	    break;
	default:
	    cerr << "Bad direction\n";
	    exit(-1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
/*/ NOTE: depthtrace assumes periodic in y, z directions
int Window::highbitwallarea(int startboundary) // count up how many bonds on wall, moving
                                               //  through regions of zeros starting from the indexed
                                               //  boundary wall 
{
    // temporarily mark up the window spin set for tracking the connected piece
    ...
    switch (startboundary) {
	case 0:			// LEFT
	    surfarea = highbitbreadthtrace(W - 1, L - 1, H - 1);
	    break;
	case 1:			// RIGHT
	    surfarea = highbitbreadthtrace(0, 0, 0);
	    break;
	default:
	    cerr << "Bad direction\n";
	    exit(-1);
