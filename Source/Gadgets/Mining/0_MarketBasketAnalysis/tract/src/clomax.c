...
{                               /* --- create a c/m prefix tree */
  ...
  cmt = (CMTREE*)calloc(1,sizeof(CMTREE)+(size_t)(size-1)*sizeof(ITEM));
  if (!cmt) return NULL;        /* create a c/m prefix tree */
  cmt->size = size;             /* and initialize its fields */
  cmt->dir  = (dir < 0) ? -1 : +1;
  cmt->max  = -2;               /* mark the tree as empty/invalid */
...
RSUPP cmt_check (CMTREE *cmt, const ITEM *items, ITEM n, RSUPP supp)
{                               /* --- check support of an item set */
  ITEM   i;                     /* buffer for an item */
  CMNODE *p;                    /* to traverse the nodes */
...
{                               /* --- print a c/m prefix tree */
  ...
  if (!cmt) {                   /* check whether tree exists */
    printf("(null)\n"); return; }
  if (cmt->max < -1) {          /* check whether tree is valid */
    printf("invalid\n"); return; }
