...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/////////////////////////   For random-bond random-field Ising model on random networks     /////////////////////////////
int main (int argc, char** argv)
{
  if(argc!=8 && argc!=9)
    {
      ...
      cout <<"\nmode = 0 : Calculate the Ground States of Random-Bond Random-Field Ising Model on a random network constructed with static model at given external field H.";
      cout <<"\nCommand format:  ./HMS_Network 0 N c R dist gamma seed H";

      cout <<"\nmode = 1 : Calculate the Avalanches in the Ground State evolution of the Random-Bond Random-Field Ising Model on a random network constructed with static model.";
      cout <<"\nCommand format:  ./HMS_Network 1 N c R dist gamma seed";
  ...
  int N        = atoi(argv[2]);             // # of nodes
  double c     = atof(argv[3]);             // mean degree 
  ...
  double gamma = atof(argv[6]);             // degree exponent used in the static model
  int seed     = atoi(argv[7]);             // random seed

  char parameters[256];
  sprintf(parameters,"N%d-c%e-R%.3lf-dist%d-gamma%.3lf-seed%d", N, c, R, dist, gamma, seed);
  ...
  if(mode==0)
    {
      ...
      cout<<"-------------------------------------------------------------------------------------------------------\n";
      cout<<"Spin Clusters in the Ground States of Random-Bond Random-Field Ising Model on a random network (CIGS-RBRFIM):\n";
      cout << " N="<< N <<" c="<< c <<" R="<< R  << " dist=" << dist << " gamma= " << gamma << " seed=" << seed << " H=" << H  
	   <<" RESOLUTION="<< (double)RESOLUTION << endl;
      MakeDirectory();


      //////////////  Find the Ground State at the field Hext=0. Begin //////////////////////////////
      State C(N, R, seed, DOWN);  
      C.Set_RandomField_RandomBond(N, c, gamma, R, seed);
  ...
  else if(mode==1) // if mode==1
    {
      ...
      cout << " N="<< N <<" c="<< c <<" R="<< R  << " dist=" << dist << " gamma= " << gamma << " seed=" << seed 
      ...
      ////////////////////////////////////////////////////////////////////////////////////////////////////////////
      State C0(N, R, seed, DOWN);          // ground state(GS) C0 with all spins DOWN, M=-N
 
      C0.Set_RandomField_RandomBond(N, c, gamma, R, seed);    // set the quenched-disorder(Gaussian random field) for all States
      C0.SetE0(DOWN);                         // C0 has internal energy = -N*D + sum(hi) with all spins DOWN

      State C1(N, R, seed, UP);            // state C1 with all spins UP, M=+N
