  ...
  if(mode==0)
    {
      ...
      //State C(D,L,R,seed);                 // allocate memory for state C
      //C.Initialize_fromfield(H);           // state C with initial all spins DOWN 
  ...
  else // if mode==1
    {
      ...
      cout << " D="<< D <<" L="<< L <<" R="<< R  << " dist=" << dist << " seed=" << seed 
	   <<" RESOLUTION="<< (double)RESOLUTION << " fastpoint= " << FASTPOINT << endl;

      MakeDirectory(D,L,R,seed);
      ...
      while(!Queue.empty()) 
	{
      ...
      State X(D,L,R,seed);                         // allocate memory for state X
	...
	{
	  LineSegment LSa = *p;
	  State A(D,L,R,seed);                         // allocate memory for state A
	  ...
	  if(j<sizeofList)
	    {
	      ...
	      State B(D,L,R,seed);            // allocate memory for state B
